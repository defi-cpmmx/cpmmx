...
interface BURNIERC20 is IERC20 {
    function burn(address addr, uint amount) external;
    function burn(uint amount) external;
}

interface ITriiad {
    function sorcery() external;
    function getMinTokenForSorcery() external view returns (uint256);
}

interface IPLASMA {
    function fusionPartTwo() external;
}
...
interface IEquinox {
    function fillLiquidityPool() external;
}
...
interface IWithdrawAll {
    function withdrawAll() external;
}

interface ITallyVotes {
